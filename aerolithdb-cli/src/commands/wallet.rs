...
pub async fn handle_wallet_command(args: WalletArgs, client: &ApiClient) -> Result<()> {
    match args.command {
...
async fn disconnect_wallet(client: &ApiClient) -> Result<()> {
...
async fn show_wallet_status(_client: &ApiClient) -> Result<()> {
